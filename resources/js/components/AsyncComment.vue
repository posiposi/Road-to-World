      ...
      <div class="col-md-12">
        ...
        <div class="card">
          ...
          <div class="card-body comment-view" v-for="(comments) in loginUserAndOwnerComments">
            ...
            <div v-if="comments.sender_id == data.sender.id">{{ comments.created_at }} {{ data.sender.nickname }} : {{
              comments.body
            }}</div>
          ...
          <div class="card-body">
            ...
            <!-- 送信ボタン -->
            <button @click="clickCommentButton" :disabled="disabled.value" ref="comment-button"
              class="btn btn-primary mt-2">送信</button>
          </div>
        </div>
      </div>
    </div>
